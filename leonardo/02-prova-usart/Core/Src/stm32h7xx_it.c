...
uint8_t __lz_serial_receive_char(void) {
	return USART3->RDR & USART_RDR_RDR_Msk;
}
...
void USART3_IRQHandler(void)
{
  /* USER CODE BEGIN USART3_IRQn 0 */
	...
	if (USART3->ISR & (USART_ISR_TXE_TXFNF | USART_ISR_TC)) {
		uint8_t c;
	...
	if (USART3->ISR & USART_ISR_TC) {
		...
		__lz_serial_end_tx_transmission();
		return;
	}

	if (USART3->ISR & USART_ISR_RXNE_RXFNE) {
		uint8_t c = __lz_serial_receive_char();
		if (!lz_queue_push(&__lz_serial_rx_buffer, c))
